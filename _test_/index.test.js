...
test('Enter keypress starts the game if difficulty is selected', () => {
  gameStarted = false;
  selectDifficultyLevel = 'Easy';
...
test('Check whether is checkWin is invoked when player wins', () => {
  const simonGame = {
    checkWin: jest.fn(),
  };
  gameStarted = true;
  ...
  selectDifficultyLevel = 'Easy';
  simonGame.checkWin();
  expect(simonGame.checkWin).toHaveBeenCalled();
});



test('Check for buttonColors when selectDifficulty = Difficult', () => {
  selectDifficultyLevel = 'Difficult';
  const event = new KeyboardEvent('keypress', { key: 'Enter' });
  document.body.dispatchEvent(event);
  ...
  expect(selectDifficultyLevel).toBe('Difficult');
  expect(console.log).toHaveBeenCalledWith("buttonColors: red,blue,green,yellow,orange,purple");
